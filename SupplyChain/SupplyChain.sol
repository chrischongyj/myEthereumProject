...
contract supplyChain {
    ...
    mapping(uint32 => uint32[]) public productTrack; //Registrations by Product ID (p_id) / Product movemenr track
    ...
    function createProduct(uint32 _ownerId, string memory _modelNumber,
    string memory _partNumber, string memory _serialNumber, uint32 _productCost) public returns (uint32) {
    ...
    modifier onlyOwner(uint32 _prodId) {
        require((msg.sender == products[_prodId].productOwner), "You are not the owner!");
        _;
